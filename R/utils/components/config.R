...
load_config <- function() {
  ...
  config$memory$mem_total <- get_mem_usage("total")
  config$memory$mem_limit <- get_mem_usage("total") * config$memory$limit
  config$memory$total_cores <- detectCores() * config$memory$core_limit
...
if (!config$simulation$example) {
  ...
  source("./example/R/utils/components/config.R")
  load_config()
}
