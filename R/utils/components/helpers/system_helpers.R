  ...
  if (os == "Windows") {
    ...
    total_rows <- as.numeric(system2("powershell",
      args = c(
        "-command",
        sprintf(
          "(Get-Content '%s' | Measure-Object -Line).Lines",
          file.path
        )
      ),
      stdout = TRUE
...
system_calc_uniq_and_rows <- function(file.path, column = NULL, sep = "\t") {
  ...
  if (os == "Windows") {
    ...
    awk_sep <- gsub("\\", "\\\\", sep, fixed = TRUE)
    cmd <- sprintf(
      "awk -F'%s' '{print $%d}' '%s' | awk '{count++; unique[$0]++} END {print count, length(unique)}'",
      awk_sep, column_number, file.path
    )
...
get_files <- function(input.dir, files.basename = NULL, step = 0, verbose = FALSE) {
  if (step > 1) {
    vebcat("Step cannot be higher than 1", color = "fatalError")
    stop("Change step to a different integer value.")
  }
  
  files <- list.files(input.dir, recursive = TRUE, full.names = TRUE)
  
  vebprint(
    remove_parent_paths(dir_files), veb = (step == 1 || verbose), 
    paste0(
      highcat("Step 1"), " | ",
      highcat("initial directories [ files: ", length(files), "] :")
    )
  )
  
    if (!is.null(files.basename)) files <- files[grepl(files.basename, files)]
      
    return(files)
}

exclude_files <- function(input.dir, exclude.dirs = NULL, exclude.files = NULL, include.files = NULL, step = 0, verbose = FALSE) {
  if (step > 6) {
  ...
  d_orig <- unlist(lapply(input.dir, function(dir) {
    dirs <- list.dirs(dir, recursive = TRUE)
    if (length(dirs) > 1) dirs[-1] else character(0)
  }))
  
  if (length(d_orig) == 0) stop("Could not find any directories.")
  ...
  vebprint(d_u, veb = (step == 2 || verbose), paste0(highcat("Step 2"), " | ", highcat("After removing exclude.dirs [", length(d),"] :")))
  if (step == 2) return(invisible())
  
  if (!is.null(include.files)) {
    f_inc <- list.files(dirname(input.dir[1]), recursive = FALSE, full.names = TRUE)
  ...
  vebprint(f_inc_u, veb = (step == 3 || verbose), paste0(highcat("Step 3"), " | ", highcat("Files included in include.files [", length(f_inc), "] :")))
  ...
  vebprint(f_u, veb = (step == 4 || verbose), paste0(highcat("Step 4"), " | ", highcat("Files in the included directories [", length(f), "] :")))
  ...
  vebprint(f_u, veb = (step == 5 || verbose), paste0(highcat("Step 5"), " | ", highcat("After removing exclude.files [", length(f), "] :")))
