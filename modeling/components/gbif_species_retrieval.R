sp_retrieve = function() {  
  ...
  cat(yellow("Writing out gbif_species_count to: \n", "outputs/gbif_retrieval_process/gbif_species_count.csv \n"))
  ...
  cat(yellow("Writing out gbif_species to: \n", "outputs/gbif_retrieval_process/gbif_species_names.csv ...and \n", "outputs/origin_lists/gbif_species.csv \n"))
  ...
  cat("Writing out similarity check csv to: \n", "outputs/similarity_check_outputs/gbif_species_check.csv \n")
  write.csv(gbif_species_check, "outputs/similarity_check_outputs/gbif_species_check.csv", row.names = F, fileEncoding = "UTF-8")
  ...
  if (a == "y") {
    source("components/gbif_occ_retrieval.R")
    sp_occ_df = occ_retrieval(gbif_species_check);
    
    source("components/gbif_sp_crop.R")
    sp_occ_cropped = crop_species(sp_occ_df);
  } 
  
  if (a == "n") {
    cat("Loading occurence data from file \n")
    sp_cavm = read.csv("outputs/gbif_retrieval_process/gbif_boreal_species.csv")
    sp_boreal = read.csv("outputs/gbif_retrieval_process/gbif_boreal_species.csv")
