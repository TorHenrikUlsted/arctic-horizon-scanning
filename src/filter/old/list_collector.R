collector = function() {
  ...
  components = list(
    aba = list(
      source = "./filtering/components/aba_wrangling.R",
      present = "./outputs/filtering/wrangling_outputs/aba/aba_arctic_present.csv",
      absent = "./outputs/filtering/wrangling_outputs/aba/aba_arctic_absent.csv"
    ),
    ambio = list(
      source = "./filtering/components/ambio_wrangling.R",
      present = "./outputs/filtering/wrangling_outputs/ambio/ambio_arctic_present.csv",
      absent = "./outputs/filtering/wrangling_outputs/ambio/ambio_arctic_absent.csv"
    ),
    gbif = list(
      source = c("./filtering/components/regions_data_import.R", "./filtering/components/gbif_species_retrieval.R"),
      species = "./outputs/filtering/gbif_retrieval_process/gbif_species_check.csv"
    ),
    glonaf = list(
      source = "./filtering/components/glonaf_wrangling.R",
      species = "./outputs/filtering/wrangling_outputs/glonaf/glonaf_species_names.csv"
    ),
    test = list(
      source = "./filtering/components/test.R",
      species = "./filtering/outputs/wrangling_outputs/test/test_species.csv"
    ) 
  )
