syncheck_dfs <- function(wrangled_dfs, column, out.dir, verbose) {
  ...
  synonym_lists <- lapply(names(wrangled_dfs), function(name) {
    ...
    file_path <- paste0("/", parent_folder, "-", child_folder, "-")
    
    if(!file.exists(paste0(cut_dir, file_path, "wfo-one.csv"))) {
      
      # Run synonym check on the species
      sp_synonyms <- check_syn_wfo(wrangled_dfs[[name]], column, paste0(out_dir, file_path))
      
      # Select best match and remove duplications
      sp_checked <- check_syn_wfo_one(sp_synonyms, column, paste0(out_dir, file_path))
      
      if (verbose) cat("Renaming file to resources/synonym-checked folder. \n")
      old_name <- paste0(out_dir, file_path, "wfo-one.csv")
      new_name <- paste0(cut_dir, file_path, "wfo-one.csv")
